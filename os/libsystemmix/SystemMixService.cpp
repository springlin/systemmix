...
#define CUSTOMER_BOOTANIMATION_VIDEO  "/data/local/boot.ts"
#define BUFFSIZE  4096
#define CUSTOMER_BOOTLOGO "/mnt/Reserve0/advert.bmp"
#define CUSTOMER_BOOTLOGO_CRC "/mnt/Reserve0/advert.crc"
...
void SystemMixService::instantiate(){
	defaultServiceManager()->addService(
    	String16("dangs.systemmix"), new SystemMixService());
}
...
int SystemMixService::setBootAnimation(const char *pathName) {
    ...
    char *p=strstr(pathName, ".zip");
    
    int dest = open(((p!=NULL)?CUSTOMER_BOOTANIMATION:CUSTOMER_BOOTANIMATION_VIDEO), O_WRONLY|O_CREAT|O_TRUNC,S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
    if (dest < 0)
    {
        close(source);
        ALOGW("creat or open BootAnimation File %s fail : errno %s",CUSTOMER_BOOTANIMATION,strerror(errno));
        return -1;
    }
    char buf[BUFFSIZE];
    int n=0;
    ...
    close(source);
    close(dest);
    chmod(((p!=NULL)?CUSTOMER_BOOTANIMATION:CUSTOMER_BOOTANIMATION_VIDEO), S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
    if (n<0)
        ret=-1;
    if(ret != 0) {
