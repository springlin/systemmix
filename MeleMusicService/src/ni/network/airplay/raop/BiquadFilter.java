...
public class BiquadFilter {
    ...
    public BiquadFilter(final int sampling_rate, final int frame_size) {
    ...
    private biquad_t biquad_lpf(final double freq, final double Q) {
        final biquad_t ret = new biquad_t();

        final double w0 = 2 * Math.PI * freq / ((float)sampling_rate / (float)frame_size);
        final double alpha = Math.sin(w0) / (2.0 * Q);

        final double a_0 = 1.0 + alpha;
    ...
    public void update(final int fill) {
        if (fill_count < 1000) {
        ...
        final double buf_delta = fill - desired_fill;
        bf_est_err = filter(bf_err_lpf, buf_delta);
        final double err_deriv = filter(bf_err_deriv_lpf, bf_est_err - bf_last_err);
    ...
    private double filter(final biquad_t bq, final double in) {
        final double w = in - bq.a[0] * bq.hist[0] - bq.a[1] * bq.hist[1];
