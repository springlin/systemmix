...
{
    public static AlacContext AlacOpenFileInput(final String inputfilename)
    {
        int headerRead;
        final QTMovieT qtmovie = new QTMovieT();
        final DemuxResT demux_res = new DemuxResT();
        final AlacContext ac = new AlacContext();
    ...
    public static void AlacCloseFile(final AlacContext ac)
    {
    ...
    public static int AlacUnpackSamples(final AlacContext ac, final int[] pDestBuffer)
    {
        int sample_byte_size;
        final SampleDuration sampleinfo = new SampleDuration();
        final byte[] read_buffer = ac.read_buffer;
        final int destBufferSize = 1024 * 24 * 3; // 24kb buffer = 4096 frames = 1 alac sample (we support max 24bps)
        int outputBytes;
        final MyStream inputStream = new MyStream();
    ...
    public static int AlacGetSampleRate(final AlacContext ac)
    {
    ...
    public static int AlacGetNumChannels(final AlacContext ac)
    {
    ...
    public static int AlacGetBitsPerSample(final AlacContext ac)
    {
    ...
    public static int AlacGetBytesPerSample(final AlacContext ac)
    {
        if (null != ac && ac.demux_res.sample_size != 0)
        {
    ...
    public static int AlacGetNumSamples(final AlacContext ac)
    {
    ...
    static int get_sample_info(final DemuxResT demux_res, final int samplenum, final SampleDuration sampleinfo)
    {
    ...
    public static void AlacSetPosition(final AlacContext ac, final long position) {
        final DemuxResT res = ac.demux_res;

        int current_position = 0;
        int current_sample = 0;
        final SampleDuration sample_info = new SampleDuration();
        for (int i = 0; i < res.stsc.length; i++) {
            final ChunkInfo chunkInfo = res.stsc[i];
            int last_chunk;
            ...
            for (int chunk = chunkInfo.first_chunk; chunk <= last_chunk; chunk++) {
                ...
                while (sample_count > 0) {
                    final int ret = get_sample_info(res, current_sample, sample_info);
                    if (ret == 0)
                        return;
