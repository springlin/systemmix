...
public class AlacDecodeUtils
{
    ...
    public static void alac_set_info(final AlacFile alac, final int[] inputbuffer)
    {
        int ptrIndex = 0;
        ptrIndex += 4; // size
        ptrIndex += 4; // frma
    ...
    static int readbits_16(final AlacFile alac, final int bits)
    {
        int result = 0;
    ...
    static int readbits(final AlacFile alac, int bits)
    {
        int result = 0;
    ...
    /* reads a single bit */
    static int readbit(final AlacFile alac)
    {
        int result = 0;
        int new_accumulator = 0;
        int part1 = 0;
    ...
    static void unreadbits(final AlacFile alac, final int bits)
    {
        final int new_accumulator = (alac.input_buffer_bitaccumulator - bits);
    ...
    static LeadingZeros count_leading_zeros_extra(int curbyte, int output, final LeadingZeros lz)
    {
    ...
    static int count_leading_zeros(final int input, final LeadingZeros lz)
    {
        int output = 0;
        int curbyte = 0;
    ...
    public static int entropy_decode_value(final AlacFile alac, final int readSampleSize, final int k, final int rice_kmodifier_mask)
    {
        ...
        else
        {
            if (k != 1)
            {
                final int extraBits = readbits(alac, k);
    ...
    public static void entropy_rice_decode(final AlacFile alac, final int[] outputBuffer, final int outputSize, final int readSampleSize, final int rice_initialhistory, final int rice_kmodifier, final int rice_historymult, final int rice_kmodifier_mask)
    {
    ...
    static int[] predictor_decompress_fir_adapt(final int[] error_buffer, final int output_size, final int readsamplesize, final int[] predictor_coef_table, final int predictor_coef_num, final int predictor_quantitization)
    {
            ...
            for (int i = predictor_coef_num + 1; i < output_size; i++)
            {
                ...
                if (error_val > 0)
                {
                    ...
                    while (predictor_num >= 0 && error_val > 0)
                    {
                        ...
                        final int sign = ((val < 0) ? (-1) : ((val > 0) ? (1) : (0)));
    ...
    public static void deinterlace_16(final int[] buffer_a, final int[] buffer_b, final int[] buffer_out, final int numchannels, final int numsamples, final int interlacing_shift, final int interlacing_leftweight)
    {
    ...
    public static void deinterlace_24(final int[] buffer_a, final int[] buffer_b, final int uncompressed_bytes, final int[] uncompressed_bytes_buffer_a, final int[] uncompressed_bytes_buffer_b, final int[] buffer_out, final int numchannels, final int numsamples, final int interlacing_shift, final int interlacing_leftweight)
    {
        if (numsamples <= 0)
            return;
        ...
        if (interlacing_leftweight != 0)
        {
            for (int i = 0; i < numsamples; i++)
            {
                ...
                {
                    final int mask = ~(0xFFFFFFFF << (uncompressed_bytes * 8));
    ...
    public static int decode_frame(final AlacFile alac, final byte[] inbuffer, final int[] outbuffer, int outputsize)
    {
        int channels;
        ...
        if (channels == 0) // 1 channel
        {
            ...
            { // so it is compressed
                final int[] predictor_coef_table = alac.predictor_coef_table;
            ...
            case 16: {
                ...
                for (int i = 0; i < outputsamples; i++)
                {
                    final int sample = alac.outputsamples_buffer_a[i];
        ...
        else if (channels == 1) // 2 channels
        {
            ...
            if (isnotcompressed == 0)
            { // compressed
                final int[] predictor_coef_table_a = alac.predictor_coef_table_a;
                int predictor_coef_num_a;
                int prediction_type_a;
                int prediction_quantitization_a;
                int ricemodifier_a;

                final int[] predictor_coef_table_b = alac.predictor_coef_table_b;
    ...
    public static AlacFile create_alac(final int samplesize, final int numchannels)
    {
        final AlacFile newfile = new AlacFile();
