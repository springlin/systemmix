...
class DemuxUtils
{
    public static int MakeFourCC(final int ch0, final int ch1, final int ch2, final int ch3)
    {
        return (((ch0) << 24) | ((ch1) << 16) | ((ch2) << 8) | ((ch3)));
    }

    public static int MakeFourCC32(final int ch0, final int ch1, final int ch2, final int ch3)
    {
        int retval = 0;
        int tmp = ch0;
    ...
    public static String SplitFourCC(final int code)
    {
        String retstr;
        char c1;
        char c2;
    ...
    public static int qtmovie_read(final java.io.DataInputStream file, final QTMovieT qtmovie, final DemuxResT demux_res)
    {
        int found_moov = 0;
        int found_mdat = 0;
    ...
    /* chunk handlers */
    static void read_chunk_ftyp(final QTMovieT qtmovie, final int chunk_len)
    {
        int type = 0;
    ...
    static void read_chunk_tkhd(final QTMovieT qtmovie, final int chunk_len)
    {
        /* don't need anything from here atm, skip */
        final int size_remaining = chunk_len - 8; // FIXME WRONG

        StreamUtils.stream_skip(qtmovie.qtstream, size_remaining);
    }

    static void read_chunk_mdhd(final QTMovieT qtmovie, final int chunk_len)
    {
        /* don't need anything from here atm, skip */
        final int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static void read_chunk_edts(final QTMovieT qtmovie, final int chunk_len)
    {
        /* don't need anything from here atm, skip */
        final int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static void read_chunk_elst(final QTMovieT qtmovie, final int chunk_len)
    {
        /* don't need anything from here atm, skip */
        final int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static void read_chunk_hdlr(final QTMovieT qtmovie, final int chunk_len)
    {
    ...
    static int read_chunk_stsd(final QTMovieT qtmovie, final int chunk_len)
    {
        int i;
        int numentries = 0;
        /* version */
    ...
    static void read_chunk_stts(final QTMovieT qtmovie, final int chunk_len)
    {
        int i;
        int numentries = 0;
        int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static void read_chunk_stsz(final QTMovieT qtmovie, final int chunk_len)
    {
        int i;
        int numentries = 0;
        int uniform_size = 0;
    ...
    static int read_chunk_stbl(final QTMovieT qtmovie, final int chunk_len)
    {
        int size_remaining = chunk_len - 8; // FIXME WRONG
     ...
     * chunk to offset box
     */
    private static void read_chunk_stco(final QTMovieT qtmovie, final int sub_chunk_len) {
        //skip header and size
        final MyStream stream = qtmovie.qtstream;
        StreamUtils.stream_skip(stream, 4);

        final int num_entries = StreamUtils.stream_read_uint32(stream);
     ...
     * sample to chunk box
     */
    private static void read_chunk_stsc(final QTMovieT qtmovie, final int sub_chunk_len) {
        //skip header and size
        final MyStream stream = qtmovie.qtstream;
        ...
        for (int i = 0; i < num_entries; i++) {
            final ChunkInfo entry = new ChunkInfo();
    ...
    static int read_chunk_minf(final QTMovieT qtmovie, final int chunk_len)
    {
        int dinf_size;
        int stbl_size;
        int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static int read_chunk_mdia(final QTMovieT qtmovie, final int chunk_len)
    {
        int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static int read_chunk_trak(final QTMovieT qtmovie, final int chunk_len)
    {
        int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static void read_chunk_mvhd(final QTMovieT qtmovie, final int chunk_len)
    {
        /* don't need anything from here atm, skip */
        final int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static void read_chunk_udta(final QTMovieT qtmovie, final int chunk_len)
    {
        /* don't need anything from here atm, skip */
        final int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    /* 'iods' */
    static void read_chunk_iods(final QTMovieT qtmovie, final int chunk_len)
    {
        /* don't need anything from here atm, skip */
        final int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static int read_chunk_moov(final QTMovieT qtmovie, final int chunk_len)
    {
        int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static void read_chunk_mdat(final QTMovieT qtmovie, final int chunk_len, final int skip_mdat)
    {
        final int size_remaining = chunk_len - 8; // FIXME WRONG
    ...
    static int set_saved_mdat(final QTMovieT qtmovie)
    {
        // returns as follows
        // 1 - all ok
