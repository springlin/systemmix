...
JNIEXPORT jint JNICALL Java_com_hisilicon_android_nfsClient_mount(JNIEnv *env, jobject obj, jstring path, jstring mPoint)
{
    FILE * fm;
    const char * Path = env ->GetStringUTFChars(path,NULL);
    const char * MPoint = env ->GetStringUTFChars(mPoint,NULL);
    ...
    FILE * pm;
    if(mPoint != NULL && strlen(MPoint)>0) { //if user set mount point, use the mPoint
        snprintf( mountPoint, sizeof(mountPoint), MPoint);
    } else {
        dirnum ++ ;
        snprintf( mountPoint, sizeof(mountPoint), "/mnt/nfsShare/nfs_share%d",dirnum);
    }
    DIR *d;
    if ((d = opendir("/mnt/nfsShare")) == NULL)
    {
    	      DB_LOG("mkdir=/mnt/nfsShare");
            mkdir("/mnt/nfsShare",0777);
    }else closedir(d);
    ...
    int mountRet = HiNSClient::NSMount(cmdStr);
    if(mountRet == -1){
        //return 0 :mount fail,but allow mount point add to Activity
        //return -1:mount fail,and not allow mount point add to Activity
        return -1;
    }
...
static const JNINativeMethod g_methods[] = {
    ...
    { "mount", "(Ljava/lang/String;Ljava/lang/String;)I", (void *)Java_com_hisilicon_android_nfsClient_mount },
...
{
    jclass clazz;
    clazz = env->FindClass("com/hisilicon/android/hinetshare/HiNfsClientManager");
    if (clazz == NULL)
        return JNI_FALSE;
